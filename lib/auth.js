// lib/auth.js — admin gate for the team-wide revenue view.
//
// No cookies anywhere, because the dashboard also runs inside a frame on the
// CRM domain and browsers block third-party cookies. The page sends the token
// as an Authorization header instead, which frames are free to do.
//
// Env vars in Vercel:
//   ADMIN_PASSWORD   the shared password for Harsh and Pratham
//   AUTH_SECRET      any long random string, used to sign tokens
//
// Creator portal links (client.html and /api/client-*) are untouched.
...
// Put this at the top of any endpoint that serves team-wide numbers.
// Returns true when the request may continue.
function requireAdmin(req, res) {
  if (!process.env.ADMIN_PASSWORD) {
    res.status(500).json({ ok: false, error: "ADMIN_PASSWORD is not set in Vercel" });
    return false;
  }
...
module.exports = { passwordOk, makeToken, tokenOk, requireAdmin };
